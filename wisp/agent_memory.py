...
# Serializes JSONL write paths (save/upsert/rotate). Concurrent turns on
# different sessions may reach upsert() from worker threads at once;
# interleaved buffered writes could otherwise mangle lines.
...
class AgentMemory:
    ...
    def __init__(self):
        ...
        # Lines this process has appended since the last load/compaction —
        # drives the duplicate-bloat compaction heuristic in upsert().
    ...
    def _load_into_cache(self) -> None:
        ...
        LAST-WINS dedupe: upsert() appends a fresh line per change instead
        of rewriting the whole file, so a session id may appear multiple
        times; the final occurrence is the current view.
        """
        self._summaries = []
        self._seen_ids = set()
        ...
        latest: dict[str, SessionSummary] = {}
        try:
            with SESSIONS_FILE.open("r", encoding="utf-8") as f:
                for line in f:
                    ...
                    try:
                        ...
                        latest[summary.session_id] = summary
                        self._seen_ids.add(summary.session_id)
                    except (json.JSONDecodeError, TypeError):
                        continue
        except OSError:
            pass
        self._summaries = list(latest.values())
        self._appended_lines = len(self._summaries)
        self._loaded = True
    ...
    def upsert(self, summary: SessionSummary) -> None:
        ...
        PERF (append-only, last-wins): instead of rewriting the entire
        file on every turn, append ONE line. Readers dedupe by session id
        taking the final occurrence. Turns the old O(total-history)
        rewrite per turn into a single line append. The file is compacted
        back to one-line-per-session via _rotate() when duplicates grow
        beyond 2x the live summary count (or _MAX_SUMMARIES is hit).
        """
        self._ensure_loaded()
        payload = json.dumps(summary.to_dict(), ensure_ascii=False)

        # Short-circuit: byte-identical to cached current view → zero I/O.
        for existing in self._summaries:
            if existing.session_id == summary.session_id:
                if json.dumps(existing.to_dict(), ensure_ascii=False) == payload:
                    return
                break
        ...
        # Update in-memory cache (replace in place keeps order stable).
        replaced = False
        for i, s in enumerate(self._summaries):
            if s.session_id == summary.session_id:
    ...
    def get(self, session_id: str) -> Optional[SessionSummary]:
        """Return the current summary for this session id, or None."""
        self._ensure_loaded()
    ...
    def _should_compact(self) -> bool:
        """True when duplicate appends have bloated the file enough that
        a rewrite is cheaper than continued appends."""
        # Lines on disk ≈ distinct summaries + replacements since load.
        duplicates = max(0, self._appended_lines - 1)
        live = len(self._summaries)
        return duplicates > max(2 * live, 32) or live > _MAX_SUMMARIES
    ...
    def _rotate(self) -> None:
        """Compact the file to one line per summary, keeping the most recent."""
        if len(self._summaries) <= _MAX_SUMMARIES and self._appended_lines <= len(self._summaries):
            return
